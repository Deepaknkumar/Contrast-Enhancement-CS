function [outfreq] = abcsc1(num,hi,lamda,gamma,dh,sm)
SN=num;
limit=10;
n=256;
cycle=4000;
...
%   Evaluate Population
for s=1:SN    
 z = fitnessfuncsc1(X(s,:),hi,lamda,gamma,dh,sm);         %evaluate Fitness
...
for a=1:cycle
    ...
    for i=1:SN
        ...
        k=ceil(rand *(SN-1));          %for random food source
                ...
                fv(i)=fitnessfuncsc1(V(i,:),hi,lamda,gamma,dh,sm);        %calculate Fitness of newly found food source
    ...
    for R=t:SN
        ...
        if rand < P(i)
                ...
                fv(i)=fitnessfuncsc1(V(i,:),hi,lamda,gamma,dh,sm);
